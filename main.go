...
func bookingsHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		if req.Status == "" {
			_, err = db.Exec("DELETE FROM bookings WHERE room_id = $1 AND booking_date = $2", roomID, req.Date)
			if err != nil {
				http.Error(w, `{"error": "Lỗi xóa dữ liệu DB"}`, http.StatusInternalServerError)
				return
			}
		} else {
			query := `
			...
			if err != nil {
				http.Error(w, `{"error": "Lỗi lưu dữ liệu DB"}`, http.StatusInternalServerError)
				return
			}
		}
...
func pricesHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		if req.Type == "base" {
			_, err := db.Exec("UPDATE rooms SET base_price = $1 WHERE room_key = $2", req.Price, req.RoomKey)
			if err != nil {
				http.Error(w, `{"error": "Lỗi DB"}`, http.StatusInternalServerError)
				return
			}
		} else if req.Type == "special" {
			_, err := db.Exec("INSERT INTO special_prices (room_key, date_str, price) VALUES ($1, $2, $3) ON CONFLICT (room_key, date_str) DO UPDATE SET price = EXCLUDED.price", req.RoomKey, req.Date, req.Price)
			if err != nil {
				http.Error(w, `{"error": "Lỗi DB"}`, http.StatusInternalServerError)
				return
			}
		} else if req.Type == "delete_special" {
			_, err := db.Exec("DELETE FROM special_prices WHERE room_key = $1 AND date_str = $2", req.RoomKey, req.Date)
			if err != nil {
				http.Error(w, `{"error": "Lỗi DB"}`, http.StatusInternalServerError)
				return
			}
